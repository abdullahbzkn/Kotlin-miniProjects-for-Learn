...
public class FruitNinja extends ApplicationAdapter implements InputProcessor {
    ...
    public void create () {
        ...
        Gdx.input.setInputProcessor(this); //kullanıcı mimiklerini anlayablyorum artık
    ...
    public void render () {
        ...
        if(lives> 0 ){
            ...
            for(int i = 0; i <lives ; i++){
                batch.draw(apple,i * 40f + 25f,Gdx.graphics.getHeight()-35f,35f,35f);
            }
            ...
            for ( Fruit fruit : fruitArray){
                if( fruit.outOfScreen()){
                    toRemove.add(fruit);
                    if(fruit.living && fruit.type == Fruit.Type.REGULAR){ //meyve yaşıyor ve regularsa canı azaltır
                        lives--;
                        holdlives = true;
                        break;
                    }
